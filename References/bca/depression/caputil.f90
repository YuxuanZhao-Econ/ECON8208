...
!                and productivity.  The model here has variable
!                capital utilization whereas the benchmark model has
...
!  FORMS:        F(k,l)    = k^theta * l
!                penalty   = zeta * min(x,0)^3
!
!  TEST CASE:    psi = 0, delta = 1, g=0, a=1
!                ===>   c = (1-beta*theta)* k^theta
!
!  INPUT FILE:   caputil.inp assumes the following order for inputs:
!
...
!  Revised, ERM, 12-12-01
   ...
   integer, parameter           :: nx=35,ns=459,na=16065,m=3,              &
                                   msim=100,mt=2000,nm=40,ldr=20000000,    &
                                   maxit=100
...
!  Parameters:
!
   open(unit=5,  file='caputil.inp')
   open(unit=7,  file='caputil.nxt')
   open(unit=8,  file='caputil.dat')
   open(unit=9,  file='cufunc.dat')
   ...
   read(5,*) i
   if (i /= ns) then
     write(*,*) 'ERROR: wrong number of exogenous states. Edit caputil.f90 '
...
!
             ...
             sum1      = sum1 +betg*pi(k,l) * ((tk2*f1t+tauk(l)*delta+     &
                         tx2*delt1)*u1t/tc2  - zeta *delt1* pent*pent)
...
!
             ...
             tsai      = tsai +  &
                         ...
                         (tk2*f1t+tauk(l)*delta+tx2*delt1)+             &
                         tk2*(f12t*thrti+f11t*txti)*u1t/tc2-            &
                         2.0*zeta*delt1*pent*tipti)

             tsaj      = betg*pi(k,l)*((u11t*tcptj-u12t*thrtj)/tc2*     &
                         (tk2*f1t+tauk(l)*delta+tx2*delt1)+             &
                         tk2*f12t*thrtj*u1t/tc2-                        &
                         2.0*zeta*delt1*pent*tiptj)
...
!
           res       = sum1 - u1*tx1/tc1 + zeta*pen*pen
...
!
     ...
     if (ierr == 1) write(*,*) 'WARNING: max iterations in pgmres in CAPUTIL.F'
