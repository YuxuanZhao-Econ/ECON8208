...
!
!  INPUT FILE:   bench.inp assumes the following order for inputs:
!
...
!  Parameters:
!
   open(unit=5,  file='bench.inp')
   open(unit=7,  file='bench.nxt')
   open(unit=8,  file='bench.dat')
   open(unit=9,  file='bfunc.dat')
   ...
   read(5,*) i
   if (i /= ns) then
     write(*,*) 'ERROR: wrong number of exogenous states. Edit bench.f90 '
...
!
     ...
     if (ierr == 1) write(*,*) 'WARNING: max iterations in pgmres in BENCH.F'
!
!    Do Newton update.
!
